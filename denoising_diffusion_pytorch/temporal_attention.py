...
class CondMlp(nn.Module):
    ...
    def forward(self, x):
        pos = self.pos_mlp(x[:, :8])
        obs = self.obs_mlp(x[:, 8:])
        
        return self.final_mlp(torch.cat([pos, obs], 1))
...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        if cond_dim > 0:
            ...
            self.cond_mlp = CondMlp(cond_dim, dim)
...
class CondTemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        #     nn.Linear(dim * 4, dim),
        # )
        self.cond_mlp = nn.Embedding(12, dim)
    ...
    def forward(self, x, mask, time, cond, use_dropout=True, force_dropout=False):
        '''
            x : [ batch x horizon x transition ]
        '''
        ...
        x = einops.rearrange(x, 'b h t -> b t h')
        x = torch.cat([x, mask[:, None, :].float().cuda()], dim=1)

        t = self.time_mlp(time)

        # Condition embedding with conditional dropout
        cond = cond.long().cuda()
