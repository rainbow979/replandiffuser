...
class KukaDataset(Dataset):
    ...
    def __init__(self, H, max_path_length=300, max_n_episodes=15600):
        ...
        qstates = qstates[:i+1]
        path_lengths = path_lengths[:i+1]
        print(f'length mean {path_lengths.mean()}, min {path_lengths.min()}, max {path_lengths.max()}')
        ## make indices
        print('Making indices')
        indices = []
        for i, path_length in enumerate(path_lengths):
        ...
        self.labels = np.stack(labels, axis=-1).astype(np.float32)
    ...
    def __getitem__(self, idx, eps=1e-7):
        path_ind, start, end = self.indices[idx]
        diff = self.labels[path_ind, end - 1] - self.labels[path_ind, start]
        #print('diff:', diff, self.labels[path_ind, end - 1])
        cond_label = np.arange(diff.shape[0])
        #print('cond label:', cond_label)
        diff_mask = (diff == 1)
        select_ind = cond_label[diff_mask]

        if len(select_ind) == 0:
            return self.__getitem__(random.randint(0, len(self.indices)-1))

        class_label = random.choice(select_ind)
        ...
        # mask = np.concatenate([mask, class_label], axis=0)

        return qstates, mask, class_label
